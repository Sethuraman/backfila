...
class BackfillRunnerTest {
  ...
  @Test fun happyCompleteRun() {
    ...
    transacter.transaction { session ->
      ...
      assertThat(partition.run_state).isEqualTo(BackfillPartitionState.RUNNING)
    }
  ...
  @Test fun getNextBatchCursorSeparateFromDb() {
    ...
    runBlocking {
      ...
      try {
        ...
        assertThat(
          fakeBackfilaClientServiceClient.getNextBatchRangeRequests.receive()
            .previous_end_key
  ...
  @Test fun stopThroughApi() {
    ...
    transacter.transaction { session ->
      val partition = session.load(runner.partitionId)
      assertThat(partition.pkey_cursor).isNull()
      assertThat(partition.run_state).isEqualTo(BackfillPartitionState.PAUSED)
    }
  }
